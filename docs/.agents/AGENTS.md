...
- **🚨 STRICT NO-AUTO-PUSH MANDATE (MANDATORY)**: NEVER run `git push` automatically. Keep all code modifications, builds, and test runs strictly local. Execute `git push` ONLY when the user explicitly instructs you to push.
...
- **Real-Time Engine**: `socket.io` server (`backend/src/socket/socketServer.js`) with `/community` room events.
- **Community Forum**: Community Doubt Board API (`/api/v1/community`) with Karma ledger (`Karma.js`), thread upvoting, comments, and solution marking.
- **Security & Validation**: `helmet`, `express-rate-limit`, `cors`, `joi` / `express-validator`, `bcrypt`, `jsonwebtoken`.
- **File Uploads & Mail**: `multer`, `cloudinary`, Brevo REST API v3.

### AI Microservice ([ai-service/](file:///d:/SOCRATES/ai-service))
- **Runtime**: Python 3.14 + FastAPI + Uvicorn. Interpreter at `ai-service/.venv/Scripts/python.exe`.
- **Hybrid ML Engine**: `HybridTutorRecommender` (`ai-service/hybrid_recommender.py`) combining TF-IDF Content-Based Cosine Similarity + SVD Matrix Factorization.
